...
text_FAIL=$("$VERAPDF" ./recursive_folder -r -v --format text 2>&1 | grep 479 | grep -w  -c FAIL) 
text_PASS=$("$VERAPDF" ./recursive_folder -r -v --format text 2>&1 | grep 479 | grep -w  -c PASS)
...
if (("$text_FAIL" == 2 )) && (( "$text_PASS" == 2 )); then
       echo ""
       echo "Expected and found:"
       echo "text_FAIL: $text_FAIL"
       echo "text_PASS: $text_PASS"
       echo "exit 0"
       exit 0
