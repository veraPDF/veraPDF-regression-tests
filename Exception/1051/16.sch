...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron"
    ...
    <sch:pattern>name = "Checking the taskResult"
        <sch:rule context="/report/jobs/job/taskResult">
            <sch:assert test='contains(exceptionMessage, "Exception: Couldn&apos;t parse stream caused by exception: Failed to parse xref table")'>
                Failed check, Expected Error: Exception: Couldn't parse stream caused by exception: Failed to parse xref table
            </sch:assert>
        </sch:rule>
    </sch:pattern>
    ...
    <sch:pattern>name = "Checking the logs"
        ...
        <sch:rule context="/report/jobs/job/logs/logMessage">
            <sch:assert test='(contains(/, "16.pdf doesn&apos;t appear to be a valid PDF.") and @occurrences = "1" and @level = "WARNING")'>Invalid logs, Expected: 
            'WARNING: 16.pdf doesn't appear to be a valid PDF.' with 1 occurrences</sch:assert>
        </sch:rule>
    </sch:pattern>
